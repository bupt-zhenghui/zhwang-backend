...
# 本地构建 arm
# docker build . -t zhwang1999/mkdocs:v2-arm
...
if [ -n "$IMAGE_ID" ]; then
  ...
  docker tag "${IMAGE_ID}" "crpi-jic5w57mqp9e5onw.cn-hangzhou.personal.cr.aliyuncs.com/zhwang/mkdocs:${TAG}"
  docker push "crpi-jic5w57mqp9e5onw.cn-hangzhou.personal.cr.aliyuncs.com/zhwang/mkdocs:${TAG}"
  echo "镜像推送阿里云仓库成功"
else
  echo "未找到镜像"
  exit 1
