...
Elasticsearch（简称 ES）是一个基于 Lucene 的分布式全文搜索引擎，常用于日志分析、全文检索、结构化数据分析等场景。

### 4.1 倒排索引（Inverted Index）

倒排索引是 ES 和 Lucene 的核心数据结构，与传统正排索引（正向索引）相对应。
...
| Doc ID | 内容 | 词项列表 |
|--------|------|----------|
| Doc 1 | 苹果 手机 便宜 | [苹果, 手机, 便宜] |
| Doc 2 | 苹果 电脑 贵 | [苹果, 电脑, 贵] |
| Doc 3 | 手机 便宜 好用 | [手机, 便宜, 好用] |

查询 `苹果` 时：遍历所有文档，检查哪些文档包含"苹果"。
...
| **结构** | 文档 → 词 | 词 → 文档 |
| **查询方式** | 遍历文档 | 直接定位词 |
| **适用场景** | 文档检索、聚合 | 关键词检索、全文搜索 |
| **查询效率** | O(N) | O(1) 或 O(log N) |
| **代表技术** | 传统数据库 | 搜索引擎（ES、Lucene） |

#### ES 中的倒排索引结构

```mermaid
flowchart LR
    D1["Doc 1: 苹果手机便宜"] --> A1["分词: 苹果"]
    D1 --> A2["分词: 手机"]
    D1 --> A3["分词: 便宜"]

    D2["Doc 2: 苹果电脑贵"] --> A4["分词: 苹果"]
    D2 --> A5["分词: 电脑"]
    D2 --> A6["分词: 贵"]
...
ES 使用 **分词器（Analyzer）** 将文本切分为词项（Term），然后构建倒排索引。分词器包含：

- **Character Filter**：字符过滤（HTML 标签、特殊字符）
- **Tokenizer**：分词器（按空格、标点切分）
- **Token Filter**：词项过滤（小写化、词干提取、停用词）

!!! tip "倒排索引优势"
    - **快速全文检索**：直接通过词定位文档<br>
    - **支持分词**：中英文分词（IK、jieba 等）<br>
    - **支持权重**：TF/IDF 加权排序<br>
    - **分布式**：倒排表可分片，横向扩展

### 4.2 相关性计算（TF/IDF）

ES 使用 TF/IDF 算法计算文档与查询的相关性得分：

| 概念 | 说明 | 公式 |
|------|------|------|
| **TF（词频）** | 词在文档中出现次数越多，得分越高 | $tf = \sqrt{freq}$ |
| **IDF（逆文档频率）** | 词在越多文档中出现，得分越低 | $idf = 1 + \log(\frac{docCount}{docFreq + 1})$ |
| **Boost** | 字段/文档权重提升 | 可配置 |
...
ES 索引（Index）由多个分片（Shard）组成，每个分片是一个独立的 Lucene 索引：
...
| **Shard** | 分片，数据分片单元 |
| **Lucene Index** | 倒排索引结构 |
| **Segment** | Lucene 内的独立索引文件 |

!!! note "写入流程"
    数据先写入内存 buffer → refresh 生成 segment → segment 刷盘 → 合并（merge）
...
| **结构化查询** | 聚合分析、仪表盘 |
| **向量检索** | ES 8.0+ 支持 dense_vector 类型 |

!!! warning "注意事项"
    - **不要拿 ES 当数据库用**：它不是事务型数据库<br>
    - **冷热数据分离**：热数据用 SSD，热数据用 HDD<br>
    - **分片数规划**：预分配过多会影响性能

## 5. Prometheus 监控告警

Prometheus 是云原生时代最主流的开源时序监控告警系统，是 Kubernetes 监控的事实标准。

- [Prometheus 全景介绍](posts/Middleware-Prometheus.md)：涵盖架构、数据模型、指标类型、PromQL、TSDB 存储、AlertManager、Spring Boot 集成等实现原理

| 主题 | 核心要点 |
|------|---------|
| **数据模型** | 多维标签 + 时间序列 |
| **采集方式** | Pull 主动拉取（HTTP） |
| **存储** | 内置 TSDB，Gorilla 压缩 |
| **查询** | PromQL 函数式查询语言 |
| **告警** | AlertManager 分组/抑制/路由 |
| **可视化** | 搭配 Grafana 使用 |